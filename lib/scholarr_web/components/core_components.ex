defmodule ScholarrWeb.CoreComponents do
  @moduledoc """
  ...
  def flash(assigns) do
    ...
    <div
      ...
      {@rest}
    >
      <p :if={@title} class="flex items-center gap-1.5 text-lg font-bold leading-6">
        <Heroicons.information_circle :if={@kind == :info} mini class="w-5 h-5" />
        <Heroicons.exclamation_circle :if={@kind == :error} mini class="w-5 h-5" />
        <%= @title %>
      </p>
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(ScholarrWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(ScholarrWeb.Gettext, "errors", msg, opts)
    end
  end
